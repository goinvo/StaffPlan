class WorkWeek < ActiveRecord::Base
  ...
  scope :for_user_and_company, lambda { |user_id, company_id|
    joins(:project).where("projects.company_id = ? AND work_weeks.user_id = ? AND cweek > ? AND year >= ?", company_id, user_id, Date.today.at_beginning_of_week.cweek, Date.today.at_beginning_of_week.year)
  }
